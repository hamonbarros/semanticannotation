	...
	public SearcherParentConcepts(String url) {
		...
		while (!searched) {	
			try {
				...
				log.error("DBPedia - N�o foi poss�vel ler o retorno da consulta a url \"" + url + "\".");
				executeTimer(log, searched);
			} catch (SAXException se) {
				log.error("DBPedia - N�o foi poss�vel instancia o leitor do retorno da url: " + url);
				executeTimer(log, searched);
			}
		}
...
//	/**
//	 * M�todo sobrescrito da superclasse que finaliza a leitura do XML de retorno.
//	 * 
//	 * @param uri A URI.
//	 * @param localName O nome local.
//	 * @param currentName A tag atual sendo lida.
...
//	}
	...
	/**
	 * M�todo sobrescrito da superclasse que faz a leitura das tags do XML de retorno.
	 * 
	 * @param ch O array de caracteres do XML retornado.
	 * @param start O indice de in�cio da leitura no array.
